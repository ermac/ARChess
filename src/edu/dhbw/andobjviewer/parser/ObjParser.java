...
import edu.dhbw.andobjviewer.models.Piece;
...
public class ObjParser {
	 ...
	 */
	public Model parse(Model model, BufferedReader is) throws IOException, ParseException {
		...
		line = is.readLine(), lineNum++)
		{
			if (line.length() > 0) {
				if (line.startsWith("#")) {
					...
					if(faces != 3) {
						throw new ParseException(model.toString(),
